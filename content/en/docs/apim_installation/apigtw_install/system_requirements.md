...
This variable is automatically set for you in EMT mode, but can be overridden by directly modifying the EMT API Gateway Dockerfiles or setting an environment variable at container runtime. See [Best practices for running API management in Docker containers](/docs/apim_howto_guides/apigw_in_containers/#setting-locale-environment-variables) for additional information.  
